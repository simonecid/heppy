...
from heppy.utils.deltar import matchObjectCollection, deltaR2
from math import sqrt

import collections

class ObjectFinder(Analyzer):
    '''Find all the particle in a certain collection within a cone from another object in another collection. 
    
    Works with any kind of object with a p4 method. 
    
    Simple example configuration:: 
    
        from heppy.analyzers.ObjectFinder import ObjectFinder
    ...
    and have several ObjectFinder instances 
    ...
    def beginLoop(self, setup):
      super(ObjectFinder, self).beginLoop(setup)
    ...
    def process(self, event):
      ...
      for collname, pdgid in self.match_collections:
        ...
        deltaR2Max = self.cfg_ana.delta_r ** 2

        for ptc in particles:
          matchname = 'matches'
          drname = 'deltaRs'
          if pdgid:
            drname = 'deltaRs_{pdgid}'.format(pdgid=pdgid)
            matchname = 'matches_{pdgid}'.format(pdgid=pdgid)
          matches = []
          deltaRs = []
          for match in match_ptcs_filtered:
            dr2 = deltaR2(ptc, match)
            if dr2 < deltaR2Max:
              matches.append(match)
              dr = sqrt(dr2)
              deltaRs.append(dr)
          setattr(ptc, drname, deltaRs)
          setattr(ptc, matchname, matches)
