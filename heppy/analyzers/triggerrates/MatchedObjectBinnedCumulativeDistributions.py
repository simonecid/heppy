...
class MatchedObjectBinnedCumulativeDistributions(Analyzer):
  '''Considers a pair of matched objects and draws the integral of the distribution of a specific quantity binned in another quantity of the matched object
  
  Example::

  I want the integral of the pt distribution of muons coming from hadron decays in jet.
  ...
  * inverted: if true, the integral will be decreasing. I.E. bin0 will contain every event and then the next bin will contain less events
  '''

  def beginLoop(self, setup):
    super(MatchedObjectBinnedCumulativeDistributions, self).beginLoop(setup)
        ...
        if value is not None:
          for x in xrange(0, len(self.cfg_ana.binning) - 1):
            ...
            if (bin_value >= self.cfg_ana.binning[x]) and (bin_value < self.cfg_ana.binning[x+1]):
              aHistogram = self.binnedHistograms[x]
              lowBin = aHistogram.FindBin(value)
              if self.cfg_ana.inverted:
                fillRange = xrange(1, lowBin + 1)
              else:
                fillRange = xrange(lowBin + 1, aHistogram.GetNbinsX() + 1)              
              for binIndex in fillRange:
                aHistogram.AddBinContent(binIndex)
    else:
